...
async function runQuery(
  ...
  for await (const message of query({
    ...
    if (message.type === 'assistant' && 'uuid' in message) {
      ...
      // Log assistant content details for dashboard visibility
    ...
    if ((message as Record<string, unknown>).type === 'tool_result') {
      const msg = message as Record<string, unknown>;
      log(`[SDK] ${msg.is_error ? '❌' : '✅'} Tool result: ${msg.tool_name || 'unknown'}`);
    }
